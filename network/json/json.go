...
// SecretKey will be used to encrypt and decrypt request data by AES if it's be set
func (p *Processor) SetSecretKey(secretKey string) {
	if p.secretKey == "" {
...
func (p *Processor) Unmarshal(data []byte) (interface{}, error) {
	if p.secretKey != "" {
		byts, _ := util.AESDecryptCBC(string(data), p.secretKey, p.secretKey)
		data = []byte(byts)
	}
...
func (p *Processor) Marshal(msg interface{}) ([][]byte, error) {
	...
	if p.secretKey != "" {
		byts, _ := util.AESEncryptCBC(string(data), p.secretKey, p.secretKey)
		data = []byte(byts)
	}
